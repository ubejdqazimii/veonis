...
export function InnovationSections({ locale }: { locale: Locale }) {
    ...
    <section className={s.dark} id="connected-intelligence" aria-labelledby="innovation-title">
      <div className={s.inner}>
        ...
        <div className={s.grid}>
          <div className={s.intro}><h2 id="innovation-title">{de ? "Komplexität wird" : "Complexity becomes"}<br /><em>{de ? "Klarheit." : "clarity."}</em></h2><p>{de ? "Ihre finanzielle Welt ist vernetzt. Unsere Beratung auch. Entdecken Sie, wie aus einzelnen Themen eine gemeinsame Richtung entsteht." : "Your financial world is connected. So is our advice. Explore how separate topics come together in one direction."}</p>
            <div className={s.steps} aria-label={de ? "Beratungsablauf entdecken" : "Explore the advisory process"}>{stages.map((item, i) => <button key={i} type="button" aria-pressed={stage === i} aria-controls="innovation-demo" onClick={() => setStage(i)}><span>0{i + 1}</span>{item[locale][0]}<ArrowRight size={16} /></button>)}</div>
            <span className={s.caption}>{de ? "Interaktive Darstellung unseres Beratungsansatzes" : "Interactive illustration of our advisory approach"}</span>
          </div>
          <div id="innovation-demo" className={s.console} aria-live="polite">
            <div className={s.consoleTop}><span><Fingerprint size={16} /> VEONIS 360°</span><span>{de ? "DER WEG ZUM ÜBERBLICK" : "THE PATH TO CLARITY"}</span></div>
            ...
            <div className={s.consoleBottom}><span>{de ? "Vernetzt betrachtet." : "Connected thinking."}</span><span>{de ? "Persönlich besprochen." : "Personal conversation."}</span></div>
          </div>
        </div>
      </div>
    </section>
    <section className={s.guide} id="your-next-step" aria-labelledby="next-step-title"><div className={s.inner}>
      <div className={s.guideHeading}><span className={s.eyebrow}><Compass size={15} /> {de ? "IHR NÄCHSTER SCHRITT" : "YOUR NEXT STEP"}</span><h2 id="next-step-title">{de ? "Was bewegt Sie gerade?" : "What’s on your mind?"}</h2><p>{de ? "Wählen Sie Ihre Situation. Entdecken Sie einen passenden Gesprächseinstieg." : "Choose your situation. Find a useful starting point for a conversation."}</p></div>
      <div className={s.choices} aria-label={de ? "Ihre Situation" : "Your situation"}>{situations.map((item, i) => {const Icon = item.icon;return <button key={i} type="button" aria-pressed={situation === i} aria-controls="situation-result" onClick={() => setSituation(i)}><Icon size={20}/>{item[locale][0]}<ArrowRight size={16}/></button>;})}</div>
      <div id="situation-result" className={s.recommendation} aria-live="polite"><div key={situation} className={s.recommendationCopy}><span className={s.eyebrow}>{de ? "DAS KÖNNEN WIR GEMEINSAM KLÄREN" : "WHAT WE CAN EXPLORE TOGETHER"}</span><h3>{choice[1]}</h3><p>{choice[2]}</p></div><div className={s.nextSteps}><ol key={situation}>{choice.slice(3).map((label, i) => <li key={label}><span>0{i + 1}</span>{label}</li>)}</ol><Link href={getLocalizedPath(locale, "contact")}>{de ? "Darüber sprechen" : "Let’s talk"}<ArrowRight size={17}/></Link></div></div>
      <p className={s.note}>{de ? "Eine erste Orientierung. Ihre persönliche Empfehlung entsteht im Gespräch." : "An initial orientation. Your personal recommendation starts with a conversation."}</p>
    </div></section>
  </>;
}
