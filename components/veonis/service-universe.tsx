...
export function ServiceUniverse({ locale }: { locale: Locale }) {
  const de = locale === "de";
    ...
    <section ref={root} id="service-universe" className={styles.section} aria-labelledby="universe-heading">
      <div className={styles.inner}>
        <header className={styles.heading}>
          <div><p className={styles.eyebrow}>VEONIS 360° · {de ? "INTERAKTIVER ÜBERBLICK" : "INTERACTIVE OVERVIEW"}</p>
            <h2 id="universe-heading">{de ? "Ihr Leben. Ihre Ziele." : "Your life. Your goals."}<br /><em>{de ? "Alles verbunden." : "Everything connected."}</em></h2></div>
          <p>{de ? "Sechs Themen. Ein Ansprechpartner. Wählen Sie, was Sie bewegt – und entdecken Sie, wie wir Sie begleiten." : "Six areas. One point of contact. Choose what matters to you and discover how we can help."}</p>
        </header>
        <div className={styles.explorer}>
          <div className={styles.visual}>
            <div className={styles.scene} data-playing={playing && visible}>
              ...
              <div className={styles.core}><span>VEONIS</span><strong>360°</strong><small>{de ? "Sie im Mittelpunkt" : "You at the centre"}</small></div>
              {topics.map((topic, i) => {
            ...
            <div className={styles.controls}>
              <button type="button" className={styles.tour} onClick={() => setPlaying(value => !value)}>{playing ? <Pause size={15} /> : <Play size={15} />} {playing ? (de ? "Tour pausieren" : "Pause tour") : (de ? "Geführte Tour starten" : "Start guided tour")}</button>
              <span>{de ? "oder ein Thema wählen" : "or choose a topic"}</span>
            </div>
          </div>
          <div id="service-description" className={styles.detail} aria-live={playing ? "off" : "polite"}>
            ...
            <Link className={styles.cta} href={getLocalizedPath(locale, "contact")}>{de ? "Kostenloses Erstgespräch" : "Free initial consultation"}<ArrowRight size={17} /></Link>
            <div className={styles.detailBottom}><Link href={getLocalizedPath(locale, "services")}>{de ? "Alle Dienstleistungen" : "All services"}<ArrowRight size={14} /></Link><div><button type="button" aria-label={de ? "Vorheriges Thema" : "Previous topic"} onClick={() => select(active - 1)}><ChevronLeft size={18} /></button><button type="button" aria-label={de ? "Nächstes Thema" : "Next topic"} onClick={() => select(active + 1)}><ChevronRight size={18} /></button></div></div>
          </div>
        </div>
        <div className={styles.footer}><span>{de ? "Noch unsicher, wo Sie starten sollen?" : "Not sure where to start?"}</span><Link href={getLocalizedPath(locale, "veonis-360-analysis")}>{de ? "Mit dem 360°-Check das Gesamtbild sehen" : "See the full picture with a 360° check"}<ArrowRight size={16} /></Link></div>
      </div>
    </section>
  );
}
