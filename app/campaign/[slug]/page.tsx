...
export default async function CampaignPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (missing) notFound();
  return <SiteShell locale="de">{campaign ? <CampaignForm campaign={campaign}/> : <section className="mx-auto max-w-xl px-6 py-24"><h1 className="text-3xl">Die Kampagne ist gerade nicht erreichbar.</h1><p className="mt-5">Bitte versuchen Sie es in einigen Minuten erneut.</p><a className="mt-6 inline-block underline" href={`/campaign/${slug}`}>Erneut versuchen</a></section>}</SiteShell>;
}
